...
from model import db, User, Quiz, QuizAttempt, Chapter, Subject
from api_utils import user_required, get_current_user
...
def get_quiz_attempts(quiz_id):
    try:
        ...
        exclude_latest = request.args.get('exclude_latest', 'false').lower() == 'true'

        # Base query for quiz attempts
        query = (
            db.session.query(QuizAttempt, Quiz)
            .join(Quiz, QuizAttempt.quiz_id == Quiz.id)
            .filter(QuizAttempt.user_id == current_user.id, QuizAttempt.quiz_id == quiz_id)
            .order_by(QuizAttempt.quiz_start_time.desc())
        )

        # If exclude_latest is true, exclude the most recent attempt
        if exclude_latest:
            latest_attempt = query.first()
            if latest_attempt:
                query = query.filter(QuizAttempt.id != latest_attempt[0].id)

        total = query.count()
        attempts = query.paginate(page=page, per_page=per_page, error_out=False)

        results = []
        for qa, q in attempts.items:
            score_percentage = (qa.total_score_earned / qa.total_score * 100) if qa.total_score > 0 else 0
            performance_tag = (
                'Outstanding' if score_percentage >= 75 else
                'Good' if score_percentage >= 50 else
            ...
            results.append({
                ...
                'score': qa.total_score_earned,
                'full_score': qa.total_score,
                'start_time': qa.quiz_start_time.strftime('%Y-%m-%d %H:%M:%S') if qa.quiz_start_time else 'N/A',
                'end_time': qa.quiz_end_time.strftime('%Y-%m-%d %H:%M:%S') if qa.quiz_end_time else 'N/A',
                'total_questions_count': qa.total_questions_count,
                'total_attempted_qn': qa.total_attempted_qn,
                'total_correct_ans': qa.total_correct_ans,
                'total_wrong_ans': qa.total_wrong_ans,
        ...
        return jsonify({'msg': f'Error fetching quiz attempts: {str(e)}'}), 500